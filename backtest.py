...
from break_and_retest_strategy import scan_dataframe, is_strong_body
...
class DataCache:
    ...
    def download_data(self, symbol: str, start_date: str, end_date: str, 
                     interval: str = "5m", force_refresh: bool = False) -> pd.DataFrame:
        """
        ...
        while current <= end:
            ...
            try:
                ...
                if not df.empty:
                    ...
                    df = df.reset_index()
                    df.columns = [col.replace("Datetime", "Datetime") if "Datetime" in col else col for col in df.columns]
    ...
    def _download_1m_data(self, symbol: str, start: datetime, end: datetime, 
            ...
            try:
                ...
                if not df.empty:
                    ...
                    if all(col in df.columns for col in required_cols):
                        ...
                        all_data.append(df)
                        print(f"Downloaded {symbol} 1m data for {chunk_start_date} ({len(df)} bars)")
                    else:
                        print(f"Warning: Missing columns for {symbol} 1m data on {chunk_start_date}")
                else:
    ...
    def _scan_continuous_data(self, df_5m: pd.DataFrame, df_1m: pd.DataFrame) -> List[Dict]:
        ...
        for day in trading_days:
            ...
            for i in range(1, len(scan_df_5m)):
                ...
                breakout_up = (
                    prev_5m['High'] <= or_high and
                    row_5m['High'] > or_high and
                    is_strong_body(row_5m) and
                    row_5m['Volume'] > row_5m['vol_ma'] * 1.0 and
                    row_5m['Close'] > or_high
                )
                breakout_down = (
                    prev_5m['Low'] >= or_low and
                    row_5m['Low'] < or_low and
                    is_strong_body(row_5m) and
                    row_5m['Volume'] > row_5m['vol_ma'] * 1.0 and
                    row_5m['Close'] < or_low
                )
                ...
                if breakout_up or breakout_down:
                    ...
                    for j in range(len(df_1m_window) - 1):
                        ...
                        returns_to_level = (
                            (breakout_up and abs(retest_1m['Low'] - breakout_level) < 0.5) or
                            (breakout_down and abs(retest_1m['High'] - breakout_level) < 0.5)
                        )
                        
                        # Check if it's a tight candle (smaller range than 5m breakout)
                        tight_candle = (retest_1m['High'] - retest_1m['Low'] < 0.75 * (row_5m['High'] - row_5m['Low']))
                        
                        # Volume should be lower than breakout (compare 1m to 5m average)
                        lower_vol = retest_1m['Volume'] < (row_5m['Volume'] / 5) * 1.5  # 5m vol / 5 bars, with 1.5x tolerance
                        ...
                        if returns_to_level and tight_candle and lower_vol:
                            ...
                            ignition = (
                                is_strong_body(ign_1m) and
                                (
                                    (breakout_up and ign_1m['High'] > retest_1m['High']) or
                                    (breakout_down and ign_1m['Low'] < retest_1m['Low'])
                                ) and
                                ign_1m['Volume'] > retest_1m['Volume']
                            )
                            
                            if ignition:
                                entry = ign_1m['High'] if breakout_up else ign_1m['Low']
                                stop = retest_1m['Low'] - 0.05 if breakout_up else retest_1m['High'] + 0.05
                                risk = abs(entry - stop)
...
def format_results(results: List[Dict]) -> str:
    ...
    output.append(f"  Overall Win Rate: {total_winners/total_trades:.1%}" if total_trades > 0 else "  Overall Win Rate: N/A")
...
Default tickers from config.json: {', '.join(DEFAULT_TICKERS)}
    ...
    parser.add_argument("--symbols", nargs="+", help=f"Stock symbols to backtest (default: all from config.json)")
    parser.add_argument("--start", required=True, help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end", required=True, help="End date (YYYY-MM-DD)")
    parser.add_argument("--initial-capital", type=float, default=10000, help="Initial capital (default: 10000)")
    parser.add_argument("--position-size", type=float, default=0.1, help="Position size as %% of capital (default: 0.1)")

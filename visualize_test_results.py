...
def main():
    ...
    p.add_argument(
        "--ticker",
        required=False,
        help="Ticker to scan (not required when using --demo or --show-test)",
    )
    p.add_argument("--out", default=None, help="Output HTML path (optional)")
    p.add_argument(
        "--demo",
        action="store_true",
        help="Use built-in unit-test demo data instead of live yfinance data",
    )
    p.add_argument(
        "--demo-scenario",
        choices=["long", "short", "long_fail", "short_fail"],
        default="long",
        help="Which demo scenario to use",
    )
    p.add_argument("--show-test", action="store_true", help="Show the latest unit test HTML output")
    p.add_argument(
        "--no-open", action="store_true", help="Do not open browser tabs (for CI/headless runs)"
    )
    args = p.parse_args()
    ...
    if args.demo:
        ...
        fig = create_chart(
            scan_df if not scan_df.empty else df, signals, title=f"Demo: {args.demo_scenario}"
        )
        # Default to saving output into logs/ with a timestamped filename
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        os.makedirs("logs", exist_ok=True)
        out_path = (
            args.out if args.out else os.path.join("logs", f"demo_{args.demo_scenario}_{ts}.html")
        )
    ...
    if not signals:
        ...
        out_path = (
            args.out
            if args.out
            else os.path.join("logs", f"{args.ticker}_break_and_retest_{ts}.html")
        )
        fig.write_html(out_path)
